...
type Div = { id: number; name: string; bnName?: string | null; url?: string | null };
type Dist = { id: number; divisionId: number; name: string; bnName?: string | null; lat?: number | null; lon?: number | null; url?: string | null };
type Upa = { id: number; districtId: number; name: string; bnName?: string | null; url?: string | null };

async function tryLoadJSON<T = unknown>(relativeFile: string): Promise<T | null> {
  try {
...
function inlineData(): { divisions: Div[]; districts: Dist[]; upazilas: Upa[] } {
  const divisions: Div[] = [
...
async function main() {
  ...
  const [divisionsJson, districtsJson, upazilasJson] = await Promise.all([
    tryLoadJSON<any[]>('divisions.json'),
    tryLoadJSON<any[]>('districts.json'),
    tryLoadJSON<any[]>('upazilas.json'),
  ]);

  const fallback = inlineData();

  const divisions: Div[] = (divisionsJson?.map(d => ({
    id: d.id, name: d.name, bnName: d.bn_name ?? d.bnName ?? null, url: d.url ?? null
  })) ?? fallback.divisions);

  const districts: Dist[] = (districtsJson?.map(d => ({
    id: d.id,
  ...
  const upazilas: Upa[] = (upazilasJson?.map(u => ({
    id: u.id,
